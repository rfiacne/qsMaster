...
- [ ] **T1.1** `turbovec_store.py` 暴露 chunk store 版本号
...
- [ ] **T1.2** 新增 `bm25_index.py` 全量 BM25 索引模块
...
- [ ] **T1.3** 改造 `hybrid_retriever.py` 使用全量 BM25 + RRF 调优 + 分数归一化
...
- [ ] **T1.4** `settings.py` RRF k 可配 + Reranker 默认开
...
- [ ] **T2.2** 新增 `query_rewriter.py` 查询改写层
...
- [ ] **T3.1** 新增 `QueryCacheConfig` + `query_cache.py`
...
- [ ] **T3.2** `querying.py` 接入查询缓存 + embed/BM25 并行
  - 文件: `src/qa/pipelines/querying.py`
...
- [ ] **T3.5** Early Exit `match_batch` 批量嵌入
...
- [ ] **T5.1** `config.example.yaml` 新增配置块
